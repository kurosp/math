...
// ====== OWNER OVERRIDE (không làm hư bot) ======
const BOT_NAME = "Btoan AI";
const OWNER_NAME = "Nguyễn Bảo Toàn";

// BẬT/TẮT: true = vẫn gọi API rồi ghi đè câu trả lời; false = không gọi API với câu hỏi dạng này
const OWNER_OVERRIDE_STILL_CALL_API = true;
...
const ABOUT_OWNER_REGEX = new RegExp(
  [
    ...
    "\\bai\\s*tao\\s*ra\\b",
    "\\bai\\s*lap\\s*trinh\\b",
    "\\bai\\s*viet\\b.*\\b(bot|ai|app|web)\\b",
  ].join("|"),
  "i"
);
...
function ownerAnswer() {
  return `Mình tên là ${BOT_NAME}. Người tạo ra mình là ${OWNER_NAME}.`;
}
// ==============================================
...
async function send(){
  ...
  try{
    // ✅ Nếu người ta hỏi kiểu: ai tạo ra mày / ai lập trình / ai chủ... => trả cố định
    if (isAboutOwner(userText)) {
      const reply = ownerAnswer();
      addBotBubble(reply);

      // vẫn lưu vào history như bình thường
      history.push({ role: "user", parts: [{ text: userText }] });
      history.push({ role: "model", parts: [{ text: reply }] });
      if (history.length > 20) history = history.slice(-20);

      return; // kết thúc luôn, khỏi gọi API
    }

    // ✅ Còn lại thì gọi API như bình thường
